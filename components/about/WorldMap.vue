<template>
  <div class="flex flex-col space-y-2">
    ...
    <div class="h-6 w-auto text-center">
      <span class="text-sm font-medium" :class="countryColor">
        {{ countryHovered }}
      </span>
    </div>
  </div>
...
const countryHovered = ref<string>('');
const countryColor = ref<string>('');

function getAllPaths() {
  const paths = document.querySelectorAll('path.visited, path.home');
  paths.forEach((path) => {
    const svgPath = path as SVGElement;

    const countryName = svgPath.dataset.name || '';
    svgPath.addEventListener('mouseover', () => {
      countryHovered.value = countryName;
      countryColor.value =
        svgPath.classList[0] === 'visited'
          ? 'text-neutral-500'
          : 'text-teal-500';
    });
    svgPath.addEventListener('mouseout', () => {
      countryHovered.value = '';
    });
  });
}
